...
class AppRuntime {
  ...
  private async snapshot(symbol: string): Promise<SymbolMarketData> {
    ...
    // Price fallback: when the websocket feed is unavailable (some web
    // browsers/networks block it), derive the price from the freshest candle.
    let lastPrice = this.prices[symbol];
    if (!isFinite(lastPrice) || lastPrice <= 0) {
      const primary = candles['15m'] ?? candles['5m'] ?? candles['1h'] ?? candles['1m'] ?? candles['4h'];
      const lastClose: number | undefined = primary?.at(-1)?.close;
      if (typeof lastClose === 'number' && isFinite(lastClose) && lastClose > 0) lastPrice = lastClose;
    }
    return { symbol, candles, lastPrice };
  }
