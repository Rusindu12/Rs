import { BOT_DEFAULTS, DEFAULT_SYMBOLS, MIN_NOTIONAL_USDT, STORAGE_KEYS } from '../config';
import type {
  BotConfig,
  Position,
  Signal,
...
export class BotEngine {
  config: BotConfig = {
    ...DEFAULT_BOT_CONFIG,
    symbols: [...DEFAULT_SYMBOLS], // start with the real watchlist (runtime may narrow/restore it)
  };
  ...
  async tick(): Promise<BotTickResult> {
    ...
    try {
      ...
      for (const symbol of this.config.symbols) {
        ...
        if (position) {
          // One symbol's failed exit must never kill the whole scan cycle.
          try {
            ...
            this.logger.log('warn', `exit deferred for ${symbol}: ${String(e).slice(0, 80)}`);
          }
  ...
  async closePosition(position: Position, reason: string, opts?: { filledRemotely?: boolean }): Promise<TradeRecord> {
    ...
    let exit = price > 0 ? price : proceedsUsdt / Math.max(1e-12, position.qty);
    if (!isFinite(exit) || exit <= 0) exit = position.entryPrice; // book-keeping fallback: PnL ≈ 0
