  ...
  return (
    <Screen>
      ...
      <Card>
        ...
        <Field
          label="Close trades after (hours)"
          value={String(config.maxHoldHours ?? 8)}
          onChangeText={(v) => setConfig({ maxHoldHours: Math.max(0, num(v)) })}
          keyboardType="numeric"
        />
        <Text style={styles.toggleHint}>
          Time stop — every BUY is eventually sold: take-profit, stop-loss, the AI turning bearish,
          or this many hours passing (0 = hold until TP/SL only).
        </Text>
