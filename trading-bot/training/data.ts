...
export async function fetchKlinesVision(symbol: string, interval: Timeframe, months = 4): Promise<Candle[]> {
  const parts: Candle[][] = [];
...
export async function loadTrainingSet(
  ...
  if (live) {
    try {
      ...
      try {
        for (const symbol of symbols) {
          // primary keeps the last ~2800 bars (≈1 month of 15m) so evals stay fast;
          // 1m/5m only need a month — alignment maps everything onto the primary
          const primary = (await fetchKlinesVision(symbol, '15m')).slice(-2800);
          const candles: Partial<Record<Timeframe, Candle[]>> = { '15m': primary };
          for (const tf of ENGINE_TIMEFRAMES) {
            if (tf === '15m') continue;
            candles[tf] = await fetchKlinesVision(symbol, tf, tf === '1m' || tf === '5m' ? 1 : 4);
          }
          sets.push({ symbol, primary, candles, live: true });
