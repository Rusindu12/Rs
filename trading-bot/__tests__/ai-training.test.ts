...
describe('trained weights model', () => {
  it('ships a valid trainedWeights.json with bounded values', () => {
    ...
    if (TRAINED.risk) {
      expect(TRAINED.risk.stopLossPct).toBeGreaterThanOrEqual(0.8);
      expect(TRAINED.risk.stopLossPct).toBeLessThanOrEqual(4);
      expect(TRAINED.risk.takeProfitPct).toBeGreaterThan(TRAINED.risk.stopLossPct);
      expect(TRAINED.risk.takeProfitPct).toBeLessThanOrEqual(10);
    }
...
describe('backtester parity with the app engine', () => {
  it('fast scorer matches generateSignal factor-by-factor', () => {
    ...
    const fast = scoreBar(prep, i, {
      ...
      slPct: weights.risk?.stopLossPct ?? 2,
      tpPct: weights.risk?.takeProfitPct ?? 4,
    });
...
describe('walk-forward optimizer', () => {
  it('returns bounded weights and respects the out-of-sample guard', () => {
    ...
    expect(chosen.slPct).toBeGreaterThanOrEqual(1);
    expect(chosen.slPct).toBeLessThanOrEqual(3.5);
    expect(chosen.tpPct).toBeGreaterThan(chosen.slPct);
    expect(chosen.tpPct).toBeLessThanOrEqual(8);
