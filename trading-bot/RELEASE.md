...
## v7.0.1 — HOTFIX: "tick failed: insufficient BTC balance / scanned 0"

From a user's engine log we found the exact crash chain that stopped trading:

- **One failed sell aborted the whole scan cycle** ("tick failed … scanned 0").
  Now every symbol's exit is isolated — a failed exit is deferred to the next
  cycle while every other symbol keeps being scanned and traded.
- **Paper-balance drift** (restored positions vs restored ledger) made sells
  throw "insufficient BTC balance" forever. Demo sells now clamp to what is
  actually held; a position with nothing held still closes cleanly (PnL booked
  at the current price) instead of crash-looping.
- **Empty default watchlist** — the engine's built-in config shipped with
  `symbols: []` and only recovered via the restore fallback. Now the engine
  always starts with the real default watchlist.
- **Web price fallback** — when the websocket feed is unavailable (some
  browsers/networks), the price is derived from the freshest kline, so
  signals, exits and manual trading all work REST-only.

If you still have stuck demo positions from before: simply open the app once —
the first scan cleans them up automatically (no reinstall needed, but the new
APK/web is recommended).
