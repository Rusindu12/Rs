...
A fifth tab with the trader's identity: gradient hero + live feed status, portfolio value
and equity sparkline, performance tiles (win rate / PnL / best trade / streak), asset
allocation, the 🧠 AI Brain card (trained + live-learning stats) and 8 unlockable
achievement badges.
...
1. **Data** — CI downloads real Binance history (data.binance.vision monthly archives, with a
   live REST fallback) — 5 timeframes for every watchlist symbol before each build
   (`training/train.ts`, public endpoints, keyless).
2. **Walk-forward optimisation** — 500+ candidate configs (factor weights, RSI/volume
   thresholds, per-timeframe multipliers **and stop-loss / take-profit levels**) are replayed
   by a fast backtester on 70 % of the data (no lookahead; fills at next-bar open, 0.1 % fees,
   SL/TP simulation). Search: seeded random search + multi-restart hill-climb.
...
5. **Traded risk is trained too** — the optimiser picks SL/TP levels the same way it picks
   weights; the bot's ATR-aware stop engine seeds from the trained levels at every entry.
6. **On-device continual learning** — while the app runs, each emitted signal is resolved
